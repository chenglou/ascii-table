...
let allCompact = List.for_all (function Compact -> true | _ -> false)
let column table index = List.map (fun row -> List.nth row index) table
let interleaveF ~first ~last ~middleF list :string list =
  let result = ref [] in
  for i = (List.length list) - 2 to 0 do
    result := (middleF (List.nth list i) (List.nth list (i + 1)) i (i + 1)) :: !result
  done;
  [first] @ !result @ [last]
...
let pad ~align ~totalWidth content =
  let freeSpace = totalWidth - (String.length (stripAnsi content)) in
  match align with
  | Left -> content ^ (String.make freeSpace ' ')
  | Right -> (String.make freeSpace ' ') ^ content
  | Center ->
    let leftSpace = freeSpace / 2 in
    (* if freeSpace is odd, give rightSpace one more space *)
    let rightSpace = freeSpace - leftSpace in
    (String.make leftSpace ' ') ^ content ^ (String.make rightSpace ' ')

let at2 listList i j = List.nth (List.nth listList i) j

let powerMap2D f grid =
  let at2Maybe listList i j =
    try Some (List.nth (List.nth listList i) j)
...
(data: string list list) =
  let tableWidth = List.length (List.nth data 0) in
  let tableHeight = List.length data in

  let styles = powerMap2D (fun cell _ -> style) data in
  let styles = List.fold_left (fun styles' extraStyle ->
  ...
  in
  let anyRow = List.hd data in
  let maxColumnsWidth = BatList.init
    (List.length anyRow)
    (fun i -> padding * 2 + BatList.max (List.map String.length (column data i)))
  in

  let paddingStr = String.make padding ' ' in
  (* first horizontal line *)
  let result = if (allCompact (List.nth styles 0)) then ref ""
  else
    ...
    for j = 0 to tableWidth - 1 do
      let maxColumnWidth = List.nth maxColumnsWidth j in
  ...
  for i = 0 to tableHeight - 1 do
    ...
    let dontDrawHorizontalBar = i < tableHeight - 1
      && allCompact (List.nth styles i)
      && allCompact (List.nth styles (i + 1))
    in
    ...
    result := !result ^ !rowText ^ (
      if i = tableHeight - 1 && allCompact (List.nth styles (tableHeight - 1)) then ""
  ...
  result := !result ^
  if (allCompact (List.nth styles (tableHeight - 1))) then ""
  else (
